...
gulp.task('default', ['build-css', 'build-js', 'inject', 'inject-global', 'inject-enough',  'webserver', 'watch', 'prepare-export']);
...
gulp.task('inject', function () {
  ...
  return target.pipe(inject(sources, { ignorePath: 'public/assets', addRootSlash: false }))
    .pipe(gulp.dest('app'));
});
...
gulp.task('inject-global', function () {
  ...
  var target = gulp.src('source/html/global.html');
  // It's not necessary to read the files (will speed up things), we're only after their paths:
  var sources = gulp.src(['public/assets/javascript/**/*.js',
                          'public/assets/stylesheets/**/*.css',
                          'public/assets/javascript/*.js',
...
gulp.task('inject-enough', function () {
  ...
  var target = gulp.src('source/html/enough.html');
  // It's not necessary to read the files (will speed up things), we're only after their paths:
  var sources = gulp.src(['public/assets/javascript/**/*.js',
                          'public/assets/stylesheets/**/*.css',
                          'public/assets/javascript/*.js',
...
gulp.task('watch', function() {
  ...
  gulp.watch('source/**', ['inject', 'inject-global', 'inject-enough', 'prepare-export']);
});

gulp.task('publish', ['build-css', 'build-js', 'inject', 'inject-global', 'inject-enough', 'prepare-export', 'export']);
gulp.task('prepare-export', function() {
  gulp.src(['app/index.html', 'app/global.html', 'app/enough.html'])
    .pipe(gulp.dest('map'));
  gulp.src(['public/assets/javascript/**/*.js',
            'public/assets/javascript/*.js'])
      .pipe(gulp.dest('map/javascript'));

  gulp.src(['public/assets/stylesheets/**/*'])
      .pipe(gulp.dest('map/stylesheets'));

  gulp.src(['public/assets/stylesheets/**/*'])
      .pipe(gulp.dest('map/stylesheets'));

  gulp.src(['public/assets/data/**/*'])
      .pipe(gulp.dest('map/data'));

      gulp.src(['public/assets/img/**/*',
                'public/assets/img/*'])
          .pipe(gulp.dest('map/img'));
});
